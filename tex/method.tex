...
Let $V_{+}:={v\in V|l(v)=1}$ with corresponding embedding $E_{+} \in R^{n_1\times d}$ and define$V_{-}, E_{-}$ analogously. Also let $K_{+-}=Cov(E_{+}^\intercal,E_{-}^\intercal)$, if $n_{+}=n_{-}=n/2$, then matrix $A$ in \eqref{eq:densray2} can be reformulated into matrix form
\begin{eqnarray}
...
\enote { hs } { you say that conceptor does not compute a
  direction , but the figure shows one }
