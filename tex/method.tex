...
To debias $X$ with conceptor one simply performs one matrix multiplication $X^{\text{debiased}}=X(I-C)$. \enote{sl}{Then we use $X^{\text{debiased}}$ as the input of the next layer.}
...
\enote{sl}{Add an "expected gender direction" to the figure, and declare "the direction found by DensRay is closer to the expected direction than Hard Debiasing."}
\begin{figure}[h]
	\centering
