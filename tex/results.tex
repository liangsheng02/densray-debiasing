...
\begin{tabular}{llccccc}
	\hline
	model & debiasing &$p(\text{he} | T)$ & $p(\text{she} | T)$& $p(\text{he} | T) +p(\text{she} | T)$ & mean(logdiff) & std(logdiff)\\
	\hline
...
\caption{\tablabel{t:templates1} BERT debiasing results on OCCTMP. \textit{bert-base} and \textit{bert-large} are the original models without debiasing. \textit{prob(he)} is
	...
	language model performance and \textit{var}, the variance of \textit{ diff}, for reference. \enote{pd}{how is var computed? is it the standard deviation as opposed for the mean which is "diff"? Maybe explain this in 3.2.1, e.g., by introducing diff(T) and then report mean and std of diff(T) across templates.} \enote{pd}{I added a suggestion for improving the table. If you like it I suggest to make other tables consistent. Also e.g., Table 2 uses different names for the debiasing methods (hard-debiasing vs. hard, etc.): I would make those names consistent.}}
\end{table}
...
This conclusion is similar to the experiment from \newcite{bau2019identifying}, which showed modifying some neurons activations in NMT did not help in controlling gender, and conjectured that gender property is very distributed on the neurons.
...
\begin{table}[h]
	...
	\begin{tabular}{ccccccc|c||c}
		...
		%-1.3&  -0.25  &0.24  &1.28  &2.19&0.45&&{\color{red}0.43&} not applicable\\
		\hline
		...
		%-0.77&  0.18& -0.01&  0.14&  0.23&  0.56& -0.13&{\color{red}0.03}&not applicable\\
		\hline
		...
		%-0.19&  0.65& 0.61&  0.57&  0.95&  1.06& -0.23&{\color{red}0.49}&not applicable\\
		\hline
	\end{tabular}
	\caption{\tablabel{t:measure1}
		Examples for quantifying bias (model: bert-base), where red(blue) denotes female(male) bias. \enote{pd}{an example for a nice graphic that can be used as inspiration is https://arxiv.org/pdf/2010.00667.pdf Table 1}}
\end{table}
...
``\text{[MASK]}\yin{是一个}\textit{occupation}\yin{。}'' We translate the occupation using Tencent Translation\footnote{https://fanyi.qq.com/} and make some manual adjustments to the translation. After removing duplicates,  302 Chinese templates remain. \enote{pd}{why are there duplicates in Chinese and not in English? Because of actress/actor in English?}
