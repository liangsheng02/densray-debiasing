...
\begin{tabular}{llccccc}
	...
	\hline
\end{tabular}
\caption{\tablabel{t:templates1} BERT debiasing results on OCCTMP, measure by diff and std described in \ref{sec:occtmp}. We also show the average sum probability $sum=prob(he|{\mathcal T})+prob(she|{\mathcal T})$ as an indication of language model performance.}
\end{table}
...
``\text{[MASK]}\yin{是一个}\textit{occupation}\yin{。}'' We translate the occupations using Tencent Translation\footnote{https://fanyi.qq.com/} and make some manual adjustments to the translation. After removing duplicates (e.g. firefighter and fireman have the same translation is Chinese), 302 Chinese templates remain.
